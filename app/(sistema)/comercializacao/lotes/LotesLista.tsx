...
export default function LotesLista({ lotes }: { lotes: any[] }) {
  ...
  return (
    <>
      <style>{`
        .lt-header  { padding: 0 32px; min-height: 88px; display: flex; align-items: center; }
        .lt-content { padding: 28px 32px; }
        @media (max-width: 640px) {
          .lt-header  { padding: 0 16px 0 56px; min-height: 60px; }
      ...
      <header className="lt-header" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, flexShrink: 0, background: C.corLt, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <i className="ti ti-stack-2" style={{ fontSize: 20, color: C.cor }} />
          </div>
          <div>
            <h1 style={{ fontSize: 19, fontWeight: 800, color: C.txt, margin: 0, lineHeight: 1.2 }}>Lotes</h1>
        ...
        <button
          ...
          style={{
            ...
            border: 'none', background: C.cor, color: '#fff', cursor: 'pointer',
          }}
        >
      ...
      {/* Conteúdo */}
      <div className="lt-content" style={{ background: C.bg, margin: '0 -2rem -2rem -2rem', minHeight: 'calc(100vh - 88px)' }}>
        ...
        {/* Lista */}
        {lotes.length === 0 ? (
          <div style={{ background: '#fff', borderRadius: 16, border: `1px solid ${C.borda}`, padding: '4rem', textAlign: 'center' }}>
            ...
            <button onClick={abrirModal} style={{ padding: '9px 20px', fontSize: 13, fontWeight: 700, borderRadius: 10, border: 'none', background: C.cor, color: '#fff', cursor: 'pointer' }}>
              + Iniciar lote
            </button>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, minWidth: 0 }}>
                    <div style={{ width: 38, height: 38, borderRadius: 9, background: st.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <i className={`ti ${st.icon}`} style={{ fontSize: 17, color: st.cor }} />
                    </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexShrink: 0 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 12, color: C.sub }}>{fmt.data(lote.created_at)}</div>
                      <div style={{ fontSize: 11, color: C.sub }}>criado em</div>
                    </div>
                  </div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 7, background: st.bg, color: st.cor, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                    ...
                    {nfeAutorizada && (
                      <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 7, background: '#F0FDF4', color: '#16A34A', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                        <i className="ti ti-file-check" style={{ fontSize: 11 }} /> NF-e {nfeAutorizada.numero_nfe}
                      </span>
                    )}
                    <i className="ti ti-chevron-right" style={{ fontSize: 15, color: C.sub }} />
                  </div>
                </div>
              )
            })}
      ...
      {modalAberto && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '28px 32px', width: 440, maxWidth: '90vw', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
              ...
              <button onClick={() => setModalAberto(false)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: C.sub, lineHeight: 1 }}>✕</button>
            </div>

            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, fontWeight: 700, color: C.txt, display: 'block', marginBottom: 6 }}>Safra <span style={{ color: '#DC2626' }}>*</span></label>
              <select value={safraId} onChange={e => setSafraId(e.target.value)}
                ...
                {safras.map(s => <option key={s.id} value={s.id}>{s.ano}{s.descricao ? ` — ${s.descricao}` : ''}</option>)}
              </select>
            </div>

            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, fontWeight: 700, color: C.txt, display: 'block', marginBottom: 6 }}>Descrição do produto <span style={{ color: '#DC2626' }}>*</span></label>
            ...
            {erro && (
              <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#DC2626', marginBottom: 16 }}>{erro}</div>
            )}

            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setModalAberto(false)} disabled={carregando}
              ...
              <button onClick={handleConfirmar} disabled={carregando || !safraId || !descricao.trim()}
                style={{ flex: 1, padding: '10px 0', fontSize: 13, fontWeight: 700, borderRadius: 9, border: 'none', background: C.cor, color: '#fff', cursor: carregando || !safraId || !descricao.trim() ? 'not-allowed' : 'pointer', opacity: carregando || !safraId || !descricao.trim() ? 0.6 : 1 }}>
