...
/**
 * Contábil — lista de devoluções (consulta).
 * Independente de comercializacao/fiscal/FiscalDevolucoesClient.tsx
 */

import { useEffect, useState } from 'react'
import { fmt } from '@/lib/fmt'
import { listarDevolucoesContabil } from './actions'
...
export default function ContabilNfeDevolucoes({ orgId }: { orgId: string }) {
  ...
  if (!dados.length) {
    return <EmptyState emoji="📋" titulo="Nenhuma devolução registrada" />
  }
    ...
    <ContentCard noPadding>
      <div style={{ overflowX: 'auto' }}>
        <table className="com-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              {['Data', 'Lote', 'Comprador', 'Kg devolvido', 'Valor', 'Chave NF-e', 'Status'].map(h => (
                <th key={h}>{h}</th>
              ))}
            </tr>
          </thead>
            ...
            {dados.map((d: any) => {
              ...
              return (
                <tr key={d.id}>
                  ...
                  <td style={{ fontWeight: 600, color: COM_C.vermelho }}>
                    {fmt.moeda(d.valor_total)}
                  </td>
                  <td style={{ fontFamily: 'monospace', fontSize: 11, color: COM_C.txtSub }}>
                    {d.chave_nfe_devolucao
                      ? `${String(d.chave_nfe_devolucao).slice(0, 20)}…`
                      : '—'}
                  </td>
                  <td>
                    <Badge
