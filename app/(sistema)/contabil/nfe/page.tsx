  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        await importarXMLNFe(orgId, xml, userId)
        ok++
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <div>
          ...
          <label htmlFor='upload-xml'
            style={{ background: COR, color: '#fff', borderRadius: 8, padding: '10px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'inline-block' }}>
                  ...
                  <tr key={nfe.id} style={{ borderBottom: '1px solid #f3f4f6', background: i % 2 === 0 ? '#fff' : '#f8f7f4' }}>
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 4,
                        background: nfe.tipo === 'entrada' ? '#dbeafe' : '#fce7f3',
                        color: nfe.tipo === 'entrada' ? '#1d4ed8' : '#9d174d' }}>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 4, background: st.bg, color: st.color }}>{st.label}</span>
                    </td>
                    <td style={{ padding: '10px 14px' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => setDetalhe(nfe)}
                          style={{ background: '#f8f7f4', color: '#374151', border: '1px solid #e5e3dc', borderRadius: 6, padding: '4px 10px', fontSize: 11, cursor: 'pointer' }}>
                          Ver
                        </button>
                        {nfe.status === 'importada' && (
                          <button onClick={() => handleIgnorar(nfe.id)}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 640, maxWidth: '95vw', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              {[
                ['Emitente', detalhe.nome_emitente],
                ['CNPJ Emitente', detalhe.cnpj_emitente],
                ['Destinatário', detalhe.nome_destinatario],
                ['Data', new Date(detalhe.data_emissao + 'T12:00:00').toLocaleDateString('pt-BR')],
                ['Total', fmt(detalhe.valor_total)],
                ['ICMS', fmt(detalhe.valor_icms)],
                ['PIS', fmt(detalhe.valor_pis)],
                ['COFINS', fmt(detalhe.valor_cofins)],
              ].map(([k, v]) => (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: '#f8f7f4' }}>
                      {['Descrição','CFOP','Qtd','Unit.','Total'].map(h => (
                        <th key={h} style={{ padding: '7px 10px', textAlign: 'left', fontWeight: 600, color: '#374151', borderBottom: '1px solid #e5e3dc' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
