...
import {
  getPlanoContas,
  getContadoresDaOrg,
  getPlanoContasExterno,
  getDePara,
  salvarDePara,
  removerDePara,
  getEscritorioDoContador,
  ...
  return (
    <div style={{ padding: 32, maxWidth: 960, margin: '0 auto' }}>
      ...
      <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 24 }}>
        Mapeie as contas do NexCoop para as contas do escritório de contabilidade.
        O plano do escritório já vem pré-carregado — basta fazer o mapeamento.
      </p>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <select onChange={e => setContadorSel(contadores.find((c: any) => c.id === e.target.value))}
            ...
            {contadores.map((c: any) => (
              <option key={c.id} value={c.id}>{c.usuarios?.nome || c.usuarios?.email}</option>
            ))}
          </select>
        </div>
      )}
      ...
      {contasExternas.length === 0 && !loading && (
        <div style={{ background: '#fffbeb', border: '1px solid #f59e0b', borderRadius: 8, padding: '12px 16px', marginBottom: 20, fontSize: 13, color: '#92400e' }}>
          ...
          Peça para ele acessar <b>Meu Escritório → Plano de Contas</b> e cadastrar as contas antes de fazer o mapeamento.
        </div>
      )}
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e3dc', padding: 20, marginBottom: 28 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr auto', gap: 10, alignItems: 'end' }}>
            <div>
              ...
              <select value={internaId} onChange={e => setInternaId(e.target.value)}
            ...
            <div>
              ...
              <select value={externaId} onChange={e => setExternaId(e.target.value)}
                style={{ width: '100%', padding: '9px 12px', border: '1px solid #e5e3dc', borderRadius: 8, fontSize: 13 }}>
                <option value=''>Selecione...</option>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 40, background: '#fff', borderRadius: 12, border: '1px solid #e5e3dc', color: '#6b7280', fontSize: 14 }}>
              ...
              {depara.map((dp: any, i: number) => (
                <tr key={dp.id} style={{ borderBottom: '1px solid #f3f4f6', background: i % 2 === 0 ? '#fff' : '#f8f7f4' }}>
                  <td style={{ padding: '10px 14px', fontSize: 13 }}>
                    <span style={{ fontWeight: 700, color: '#635BFF' }}>{dp.conta_interna?.codigo}</span>
                    {' — '}{dp.conta_interna?.nome}
                  </td>
                  <td style={{ padding: '10px 14px', fontSize: 18, color: COR, fontWeight: 700, textAlign: 'center' }}>→</td>
                  <td style={{ padding: '10px 14px', fontSize: 13 }}>
                    <span style={{ fontWeight: 700, color: COR }}>{dp.conta_externa?.codigo}</span>
                    {' — '}{dp.conta_externa?.nome}
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <button onClick={() => handleRemover(dp.id)}
                      style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fca5a5', borderRadius: 6, padding: '4px 12px', fontSize: 12, cursor: 'pointer' }}>
                      Remover
                    </button>
                  </td>
                </tr>
