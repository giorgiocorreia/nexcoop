...
export default function OrganizacaoPerfilClient({ org }: { org: Organizacao }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/organizacoes/perfil', {
      method: 'PATCH',
        ...
        <SectionCard titulo="Identificação">
          <FormGrid cols="1fr 160px">
            <Field label="Nome completo" required>
              <InpText value={form.nome} onChange={set('nome')} />
            </Field>
            <Field label="Sigla / Nome curto">
              <InpText value={form.nome_curto} onChange={set('nome_curto')} placeholder="Ex: COOPAIBI" />
            </Field>
          </FormGrid>
          <FormGrid cols="1fr 1fr">
            ...
            <Field label="Tipo">
              <div style={{ ...inp, background: '#f0eeea', color: '#888', cursor: 'default' }}>
