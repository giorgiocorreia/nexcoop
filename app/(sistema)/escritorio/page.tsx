      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#6b7280', margin: '4px 0 0' }}>
            Dados do escritório de contabilidade e plano de contas centralizado
          </p>
        </div>
        {!editando && (
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e3dc', padding: 24, marginBottom: 24 }}>
        {editando ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              ['Razão Social *', razaoSocial, setRazaoSocial, 'Ex: Salomão Contabilidade Ltda'],
              ['CNPJ', cnpj, setCnpj, '00.000.000/0001-00'],
              ['CRC Responsável', crc, setCrc, 'Ex: CRC-BA 012345/O-1'],
              ['E-mail de contato *', email, setEmail, 'contador@escritorio.com.br'],
              ['Telefone', telefone, setTelefone, '(73) 99999-9999'],
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {[
              ['Razão Social', escritorio?.razao_social],
              ['CNPJ', escritorio?.cnpj],
              ['CRC Responsável', escritorio?.crc_responsavel],
              ['E-mail', escritorio?.email_contato],
      ...
      {escritorio && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          {[
            ...
            { titulo: 'Organizações Clientes', desc: 'Veja todas as cooperativas que te vincularam', href: '/escritorio/organizacoes', icon: '🏢' },
          ].map(card => (
            <a key={card.titulo} href={card.href}
              style={{ display: 'block', background: '#fff', borderRadius: 12, border: '1px solid #e5e3dc', padding: 20, textDecoration: 'none' }}>
