...
export default function PlanoContasExternoPage() {
  ...
  const porTipo = TIPOS.reduce((acc, t) => {
    acc[t] = contas.filter(c => c.tipo === t)
    return acc
  }, {} as Record<string, any[]>)

  return (
    <div style={{ padding: 32, maxWidth: 960, margin: '0 auto' }}>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1a1a2e', marginBottom: 4 }}>Plano de Contas do Escritório</h1>
      <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 24 }}>
        Cadastre aqui as contas do seu sistema contábil. Este plano será reutilizado em todas as cooperativas que você atende.
      </p>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e3dc', padding: 20, marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <input value={codigo} onChange={e => setCodigo(e.target.value)} placeholder='Código (ex: 1.1.01.0001)'
        ...
        <p style={{ fontSize: 11, color: '#9ca3af', marginTop: 10 }}>
          Este plano de contas é centralizado. Qualquer conta adicionada aqui estará disponível automaticamente em todas as cooperativas que você atende.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 60, background: '#fff', borderRadius: 12, border: '1px solid #e5e3dc', color: '#6b7280' }}>
          Nenhuma conta cadastrada ainda. Adicione as contas do seu sistema contábil acima.
        </div>
      ) : (
        TIPOS.filter(t => porTipo[t]?.length > 0).map(t => (
          <div key={t} style={{ marginBottom: 20 }}>
            <h3 style={{ fontSize: 13, fontWeight: 700, color: COR, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>
              {TIPO_LABEL[t]} ({porTipo[t].length})
            </h3>
                ...
                <tbody>
                  {porTipo[t].map((c: any, i: number) => (
                    <tr key={c.id} style={{ borderBottom: '1px solid #f3f4f6', background: i % 2 === 0 ? '#fff' : '#f8f7f4' }}>
                      <td style={{ padding: '10px 14px', fontWeight: 700, color: COR, fontSize: 13, width: 160 }}>{c.codigo}</td>
