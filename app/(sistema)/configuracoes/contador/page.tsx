...
export default function ContadorConfigPage() {
  ...
  return (
    <div style={{ padding: 32, maxWidth: 720, margin: '0 auto' }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e3dc', padding: 24, marginBottom: 28 }}>
        ...
        {sucesso && (
          <div style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: 8, padding: '10px 14px', marginBottom: 14, color: '#166534', fontSize: 13 }}>
            {sucesso}
          </div>
        )}
        {erro && (
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <input value={email} onChange={e => setEmail(e.target.value)}
            placeholder='E-mail do contador (deve ter conta no NexCoop)'
        ...
        <p style={{ fontSize: 11, color: '#9ca3af', marginTop: 10 }}>
          O contador precisa ter uma conta criada no NexCoop. Após vinculado, ele verá esta organização no painel dele.
        </p>
      </div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e3dc', overflow: 'hidden' }}>
          {contadores.map((c: any, i: number) => (
            <div key={c.id} style={{
              ...
              padding: '14px 20px', borderBottom: i < contadores.length - 1 ? '1px solid #f3f4f6' : 'none',
              background: i % 2 === 0 ? '#fff' : '#f8f7f4',
            }}>
              <div>
                <p style={{ margin: 0, fontWeight: 600, fontSize: 13 }}>{c.usuarios?.nome || c.usuarios?.email || '—'}</p>
                <p style={{ margin: 0, fontSize: 12, color: '#6b7280' }}>
                  {c.usuarios?.email} · {c.nivel === 'contador' ? 'Contador' : 'Auxiliar Contábil'}
                </p>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{
                  ...
                  color: c.ativo ? '#166534' : '#6b7280',
                }}>
                  {c.ativo ? 'Ativo' : 'Inativo'}
                </span>
                <button onClick={() => handleToggle(c.id, c.ativo)}
                  style={{
                    padding: '6px 14px', border: `1px solid ${c.ativo ? '#fca5a5' : COR}`,
                    borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer',
                    background: '#fff', color: c.ativo ? '#dc2626' : COR,
                  }}>
