...
export default function RelatorioVendasClient({ vendas, operadores }: { vendas: Venda[]; operadores: Operador[] }) {
  ...
  return (
    <>
      <style>{`
        .rv-header  { padding: 0 32px; min-height: 88px; display: flex; align-items: center; }
        .rv-content { padding: 28px 32px; }
        @media (max-width: 640px) {
          .rv-header  { padding: 0 16px 0 56px; min-height: 60px; }
          .rv-content { padding: 16px; }
        }
      `}</style>

      <header className="rv-header" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, flexShrink: 0, background: C.laranjaLt, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <i className="ti ti-chart-bar" style={{ fontSize: 20, color: C.laranja }} />
          </div>
          <div>
            <h1 style={{ fontSize: 19, fontWeight: 800, color: C.txt, margin: 0, lineHeight: 1.2 }}>Relatório de Vendas</h1>
        ...
        <button onClick={() => exportCSV(filtradas)} style={{ padding: '8px 14px', background: '#fff', color: C.txt, border: `1px solid ${C.borda}`, borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          <i className="ti ti-download" style={{ fontSize: 15 }} />
          Exportar CSV
        </button>
      </header>

      <div className="rv-content" style={{ background: C.bg, margin: '0 -2rem -2rem -2rem', minHeight: 'calc(100vh - 88px)' }}>
        ...
        <div style={{ background: '#fff', border: `1px solid ${C.borda}`, borderRadius: 12, padding: '14px 20px', marginBottom: 16, display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
          ...
          <button onClick={() => { setFiltroInicio(""); setFiltroFim(""); setFiltroForma(""); setFiltroOp(""); }} style={{ fontSize: 12, padding: '6px 12px', borderRadius: 8, border: `1px solid ${C.borda}`, background: 'transparent', color: C.txtSub, cursor: 'pointer' }}>
            Limpar
          </button>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          {[
            ...
            <div key={c.label} style={{ background: '#fff', border: `1px solid ${C.borda}`, borderRadius: 8, padding: '8px 14px', display: 'flex', gap: 8, alignItems: 'baseline' }}>
              <span style={{ fontSize: 11, color: C.txtSub }}>{c.label}</span>
              <span style={{ fontSize: 14, fontWeight: 700, color: C.txt }}>{c.value}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: '#fff', border: `1px solid ${C.borda}`, borderRadius: 12, overflow: 'hidden' }}>
              ...
              <tbody>
                {filtradas.map((v, i) => (
                  <tr key={v.id} style={{ borderBottom: "1px solid #f5f5f4", background: i % 2 === 0 ? "#fff" : "#fafaf9" }}>
                    ...
                    <td style={{ padding: "11px 14px", fontSize: 12, color: C.txtSub, fontFamily: "monospace" }}>{v.hora}</td>
                    <td style={{ padding: "11px 14px", fontSize: 12, fontWeight: 600, color: C.laranja }}>{v.num}</td>
                    <td style={{ padding: "11px 14px", fontSize: 12 }}>{v.operador}</td>
                    <td style={{ padding: "11px 14px", fontSize: 11 }}>
                      <span style={{ background: v.forma === "credito_cooperado" ? "#e0f2fe" : "#f5f5f4", color: v.forma === "credito_cooperado" ? "#0369a1" : C.txtSub, padding: "2px 7px", borderRadius: 5, fontWeight: 600, whiteSpace: "nowrap" }}>{fmtForma(v.forma)}</span>
                    </td>
          ...
          {filtradas.length === 0 && (
            <div style={{ padding: "40px", textAlign: "center", color: C.txtSub, fontSize: 13 }}>Nenhuma venda encontrada.</div>
          )}
