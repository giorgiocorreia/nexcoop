  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 900 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 13, color: '#78716c', fontWeight: 600 }}>Conferência de Caixa</span>
      </div>

      {pendentes > 0 && (
        <div style={{ background: '#fef9c3', border: '1px solid #fde047', borderRadius: 8, padding: '10px 16px', marginBottom: 20, fontSize: 13, color: '#854d0e', fontWeight: 500 }}>
          {pendentes} caixa{pendentes !== 1 ? 's' : ''} aguardando conferência
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {caixas.map(c => {
          ...
          return (
            <div key={c.id} style={{ background: '#fff', border: `1px solid ${aberto ? '#E07B30' : '#e5e3dc'}`, borderRadius: 12, overflow: 'hidden', transition: 'border-color 0.15s' }}>
              <div
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <i className="ti ti-cash-register" style={{ fontSize: 20, color: '#E07B30' }} />
                  <div>
              ...
              {aberto && (
                <div style={{ borderTop: '1px solid #f0eeea', padding: '20px' }}>
                  {carregando ? (
                    <div style={{ textAlign: 'center', color: '#6b7280', fontSize: 13, padding: '20px 0' }}>Carregando...</div>
                  ) : (
                    <>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 20 }}>
                        {[
                          ...
                          return (
                            <div key={item.label} style={{ background: '#f8f7f4', borderRadius: 8, padding: '12px' }}>
                              <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 6 }}>{item.label}</div>
                              <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 2 }}>Sistema: {fmtReal(item.sistema)}</div>
                              <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 4 }}>Informado: {item.fisico != null ? fmtReal(item.fisico) : '—'}</div>
                              {item.fisico != null && (
        ...
        {caixas.length === 0 && (
          <div style={{ textAlign: 'center', padding: '48px', color: '#6b7280', fontSize: 13, background: '#fff', border: '1px solid #e5e3dc', borderRadius: 12 }}>
