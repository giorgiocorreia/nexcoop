...
export default function MensalidadesLista({ mensalidades, cooperadoMap }: Props) {
  ...
  // Mensalidades com status efetivo calculado
  ...
  // Resumo (sobre todos os dados, não os filtrados)
  ...
  return (
    <div style={{ maxWidth: '1100px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: '600', color: '#1a1a1a', margin: 0 }}>Mensalidades</h1>
          <p style={{ fontSize: '14px', color: '#888', marginTop: '4px' }}>Cobranças mensais dos cooperados</p>
        </div>
        <button
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '13px', fontWeight: '500', color: '#1a1a1a' }}>
                            {coop?.nome_completo ?? 'Cooperado removido'}
                          </div>
                          {coop?.cpf && (
      ...
      {filtrados.length > 0 && (
        <p style={{ fontSize: '12px', color: '#aaa', marginTop: '8px', textAlign: 'right' }}>
          {filtrados.length} de {mensalidades.length} cobrança{mensalidades.length !== 1 ? 's' : ''}
        </p>
      )}
    </div>
  )
