...
export default function GerarMensalidadesPage() {
  ...
  // Reset preview quando parâmetros mudam
  ...
  async function handleGerar() {
    ...
    if (novos.length === 0) { setErro('Nenhuma cobrança nova a gerar — todos os cooperados já têm mensalidade para este mês.'); return }
    ...
    if (payload.length === 0) {
      setErro('Todos os valores estão em R$ 0,00. Informe um valor padrão ou a quota-parte dos cooperados.')
      setGerando(false)
      return
    }
    ...
    <div style={{ maxWidth: '820px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1.5rem' }}>
        ...
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#888', marginTop: '2px' }}>Cria cobranças para todos os cooperados ativos e probatórios</p>
        </div>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e3dc', borderRadius: '12px', padding: '1.25rem', marginBottom: '1.25rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
          ...
          <InputGroup label="Valor padrão (R$)" dica="Usado quando o cooperado não tem quota-parte definida.">
        ...
        <div style={{ marginTop: '1.25rem', display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {carregando && <span style={{ fontSize: '12px', color: '#888' }}>Carregando cooperados…</span>}
        </div>
      </div>
        ...
        <>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e3dc', borderRadius: '12px', overflow: 'hidden', marginBottom: '1.25rem' }}>
            <div style={{ padding: '10px 16px', background: '#fafaf8', borderBottom: '1px solid #e5e3dc', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 120px', gap: '0' }}>
              {['Cooperado', 'Status', 'Quota-parte', 'Valor cobrança'].map(h => (
