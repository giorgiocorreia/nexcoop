...
| 092 | **Aplicada em produção 06/08/2026.** `recibos` (NOVA) — recibos avulsos gerados na tela Impressos, PDF A4 com 2 vias e linha de corte. Uma linha por recibo emitido; **o PDF nunca é armazenado**, é sempre regenerado a partir da linha. Snapshot do que a pessoa assinou (`pessoa_nome`, `pessoa_cpf`, `valor`, `descricao`, `tipo`, `direcao`) para a 2ª via sair idêntica; o cabeçalho da cooperativa é lido **ao vivo** de `organizacoes`. `direcao` ('recebemos' \| 'pagamos') define o texto impresso e quem assina — derivada do tipo em `lib/pdf/recibo-utils.ts`, mas gravada porque a regra do tipo pode mudar. Recibo emitido **nunca** é editado nem apagado: errou → `cancelado_em`, e o número fica queimado. + `organizacoes.ultimo_numero_recibo` (default 0), reservado por **compare-and-swap** na action `gerarRecibo` (`UPDATE ... WHERE ultimo_numero_recibo = <valor lido>`), com `uq_recibo_numero_por_org` como rede final — talão independente de `ultimo_numero_ficha`. RLS: SELECT membros da org; **nenhuma policy de escrita** (emitir queima numeração, é ato de servidor via `createAdminClient()`) |

| 093 | **Aplicada em produção 06/08/2026.** `recibos.competencia` (date NULL) — mês a que o recibo se refere, **não** a data de emissão (`emitido_em`): emitir em 03/09 um recibo da competência 2026-08-01 é o caso normal, e é a competência que a contabilidade usa pra alocar a despesa. Dia sempre 01, garantido por `CHECK recibos_competencia_dia_1` — sem isso metade das linhas gravaria o dia da emissão e a comparação por igualdade quebraria. `date` em vez de texto "MM/AAAA" para permitir filtro/agrupamento por período (+ `idx_recibos_org_competencia`, parcial em `competencia IS NOT NULL`). NULL é o default (doação, adiantamento não têm competência) e nesse caso a linha não é impressa no PDF. **Atenção no código:** converter com as funções string-to-string de `lib/pdf/recibo-utils.ts` (`formatarCompetencia`/`competenciaParaData`), nunca com `new Date('AAAA-MM-01')` — em fuso negativo isso volta pro mês anterior |

| 094 | **Aplicada em produção** (criada 06/08/2026; presença confirmada por diagnóstico em 07/08/2026 — `loja_sangrias.forma_pagamento` existe no banco). Blindagem do caixa da **Loja**, trazendo pra ela duas travas que a Comercialização já tinha. (a) `loja_caixas_unico_aberto_por_usuario` — índice único parcial em `(org_id, usuario_id) WHERE status = 'aberto'`, equivalente da 090. **Continua valendo que Loja e Comercialização são independentes**: o mesmo usuário pode ter `loja_caixas` aberto E `sessoes_caixa` aberta ao mesmo tempo. (b) `loja_sangrias.forma_pagamento` (default `'especie'`, mesmo CHECK da 063) — só aporte em espécie é cédula na gaveta; aporte por pix/cartão não passa pelo caixa físico e não pode compor o saldo que o operador conta no fechamento, senão infla a custódia dele e reaparece como valor de abertura do dia seguinte (bug real na Comercialização, commit `fc34370`). Default `'especie'` é correto no histórico: não existia UI pra registrar aporte de outra forma na Loja. Motivo da migration: `getCaixaLojaAbertoDoOperador` e `abrirCaixaLoja` usavam `.maybeSingle()`, que **erra** com 2 linhas — o erro era engolido e virava "caixa fechado" com caixa aberto, levando o operador a abrir mais um (mesmo ciclo do incidente do Luan, 090). Diagnóstico rodado em produção 06/08: **zero** `loja_caixas` abertos, o índice aplica limpo |

| 095 | **Aplicada em produção 07/08/2026.** `site_conteudos` ganha os campos que faltavam pra portar `noticias.php` e `videos.php` do site da COOPAIBI: `slug` (URL própria da notícia), `conteudo` (corpo integral em HTML — `descricao` continua sendo o resumo/chamada), `youtube_id` (id já extraído de `url_externa`, evita reparsear a cada exibição), `categoria` (agrupamento na barra lateral de Vídeos) e `destaque` (bool NOT NULL default false). `slug` é **único por organização** via índice parcial `uq_site_conteudos_slug_por_org` WHERE slug IS NOT NULL — duas orgs podem ter "assembleia-2026" sem colidir, e vídeo/evento seguem sem slug. CHECK `site_conteudos_slug_formato` só aceita minúsculas/dígitos separados por hífen (identificador de URL). + `idx_site_conteudos_org_tipo_ativo` (org, tipo, ativo, destaque DESC, ordem), sem o qual cada visita ao site varre a tabela |

| 096 | **Aplicada em produção 07/08/2026.** `site_leads` (NOVA) — interessados vindos dos formulários do site público (`cooperado`, `parceria`, `agendamento_cacau`). Substitui os `enviar-*.php` do cPanel, que faziam `mail()` e morriam na caixa de entrada: ninguém sabia quantos leads chegaram no mês nem quais foram respondidos. **Motivo de ser agora:** com a virada de DNS o cPanel deixa de servir o site e os `.php` morrem junto. Campos comuns aos três formulários viram coluna (`nome`, `email`, `telefone`, `mensagem`) pra dar pra listar/ordenar/buscar sem abrir o jsonb; o resto, que difere muito entre eles, vai em `dados` jsonb — promover tudo a coluna deixaria a tabela cheia de nulos. `status` ('novo' \| 'em_contato' \| 'convertido' \| 'descartado', default 'novo') + `observacoes` pro acompanhamento; `origem`/`user_agent`/`ip` pra saber que página converte e investigar envio automatizado. RLS: SELECT e UPDATE por membros da org; **nenhuma policy de INSERT, de propósito** — quem grava é a rota (`app/(site-org)/[slug]/enviar/[tipo]`) via `createAdminClient()`, que ignora RLS. Abrir INSERT pra `anon` permitiria inundar a tabela sem passar pela validação e pelo limite de 2000 chars por campo que ficam na rota. Substitui o paliativo anterior, que gravava lead em `site_conteudos` com `tipo='pagina'` e `ativo=false` |

**Próxima migration:** 097
